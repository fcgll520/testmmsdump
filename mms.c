...
static int mmst_valid_proto (char *proto) {
	...
	while(mmst_proto_s[i]) {
		if (!strcmp(tolower(proto), mmst_proto_s[i])) {
			return 1;
		}
		i++;
	}
...
//		lprintf ("invalid url\n");
	...
	if (uri->query) {
		size_t plen = strlen(c);
		size_t qlen = strlen(uri->query);
...
//	lprintf("send command 0x15\n");
	...
	fwrite(this->asf_header,this->asf_header_len,1,data);

	dSize = (double)this->asf_header_len / 1024 / 1024;
	printf("%f\n", dSize);

	this->ftime_len=0;//initial foward time
...
double mms_read (mms_io_t *io, mms_t *this, void *data, int len) {
	int total;
	total = 0;
	while (total < len &&!this->eos) {// 
		...
		if (bytes_left == 0) {
			...
			if(this->buf[0]==0x82&&this->buf[1]==0x00&&this->buf[2]==0x00)
			{
				if(this->buf[3]==0x48)
					time_len=LE_32(this->buf+8);
				if(this->buf[3]==0x40||this->buf[3]==0x41)
					time_len=LE_32(this->buf+7);
				if(this->buf[3]==0x09)
					time_len=LE_32(this->buf+6);
				if(this->ftime_len==0)
					this->ftime_len=time_len;
				this->time_len+=((time_len-this->ftime_len)/1000.0);
				this->ftime_len=time_len;
				printf("time%f\n",this->time_len);
			}

			fwrite(this->buf,this->buf_size,1,data);
			dSize += (double)this->buf_size / 1024 / 1024;
			printf("%f\n", dSize);
